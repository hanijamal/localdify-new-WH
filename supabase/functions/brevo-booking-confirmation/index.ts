...
const templatesByLang = {
    ...
    ar: {
        confirmation: {
            ...
            body: "<div dir=\"rtl\" style=\"text-align: right; font-family: 'Amiri', serif;\"><p>مرحباً {{customerName}}،</p><p>هذه رسالة لتأكيد تفاصيل حجزك:</p><ul style='list-style: none; padding: 0; margin-right: 0;'><li><strong>الخدمة:</strong> {{serviceName}}</li><li><strong>السعر:</strong> {{price}}</li><li><strong>التاريخ والوقت:</strong> {{date}} الساعة {{time}}</li><li><strong>مقدم الخدمة:</strong> {{staffMemberName}}</li><li><strong>الفرع:</strong> {{locationName}}</li><li><strong>العنوان:</strong> {{locationAddress}}</li></ul><p>نتطلع لرؤيتك قريباً!<br/>{{businessName}}</p><p style=\"text-align:center;margin-top:20px;font-size:14px;color:#999999;\">{{socialsLinks}}</p><p style=\"text-align:center;font-size:12px;color:#999999;margin-top:20px;\">بدعم من <a href=\"{{appLink}}\" style=\"color:#644a40;text-decoration:none;\">localDify</a></p></div>"
        }
    },
    fr: {
        confirmation: {
