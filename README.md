...
DECLARE
    ...
    hook_url_3 text := 'https://' || split_part(show(current_setting('app.supabase.url')),'//',2) || '/functions/v1/whatsapp-message-processor';
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM pg_net.http_request_queue WHERE url::text = hook_url_3) THEN PERFORM net.http_delete(url := hook_url_3::URI); END IF;
END;
$$;
...
DROP TRIGGER IF EXISTS on_booking_insert_whatsapp ON public.bookings;
...
-- Create the webhook for WhatsApp confirmations (INSERT only)
CREATE TRIGGER on_booking_insert_whatsapp
AFTER INSERT ON public.bookings
FOR EACH ROW EXECUTE PROCEDURE supabase_functions.http_request(
  'https://<YOUR_PROJECT_REF>.supabase.co/functions/v1/whatsapp-message-processor',
  'POST',
  '{"Content-Type": "application/json", "Authorization": "Bearer @supa-secrets.WHATSAPP_WEBHOOK_SECRET"}',
  '{}',
  '1000'
);
```
...
Your webhooks are now correctly and securely configured.
...
1.  Go to your Supabase Dashboard: **Edge Functions** > **send-reminders**.
...
FROM cron.job
WHERE command LIKE '%/functions/v1/send-reminders%';

-- Schedule the cron job to run daily at 9 AM UTC
SELECT cron.schedule(
  'daily-reminders', -- A unique name for the job
...
2.  **To test reminders:** You can manually trigger the cron job. Go to **Edge Functions** > **send-reminders** > **Cron Jobs**, click the three dots (...) on your job, and select **Trigger now**.
3.  Go to your Supabase Dashboard: **Edge Functions** > (either `brevo-booking-confirmation`, `whatsapp-message-processor` or `send-reminders`).
