...
import { getTicketByIdForAdmin, addMessageToTicket, updateTicketStatus } from '../../services/supabaseService';
...
const AdminTicketDetail: React.FC = () => {
    ...
    const fetchTicket = async () => {
        ...
        setLoading(true);
        try {
            const fetchedTicket = await getTicketByIdForAdmin(ticketId);
    ...
    if (!ticket) {
        return (
            <div className="text-center">
                <p className="text-muted-foreground">{t('noTicketsFound')}</p>
                <Link to="/admin/support" className="mt-4 inline-block">
                    <Button>{t('backToTickets')}</Button>
                </Link>
            </div>
        );
    }
    ...
    return (
        <div className="max-w-4xl mx-auto">
            <Link to="/admin/support" className="inline-flex items-center text-sm font-medium text-muted-foreground hover:text-foreground mb-4">
            ...
            <Card>
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-foreground">{ticket.subject}</h1>
                             <p className="text-sm font-semibold text-muted-foreground">
                                {t('ticketBusiness', { name: ticket.businessName || 'N/A' })}
                            </p>
                            <p className="text-sm text-muted-foreground">
                                {t('ticketFrom', { name: ticket.userName, email: ticket.userEmail })}
                            </p>
                            <p className="text-sm text-muted-foreground">
                                {t('ticketOpened', { date: new Date(ticket.createdAt).toLocaleString() })}
                            </p>
                        </div>
                ...
                <CardContent className="p-4 sm:p-6 h-[50vh] overflow-y-auto bg-muted/30">
                    <div className="space-y-6">
                         {ticket.messages && ticket.messages
                            ...
                            .map((msg) => {
                                const isAdminMessage = msg.userId === user?.id;
                                const isUserMessage = msg.userId === ticket.userId;
                                return (
                                    <div key={msg.id} className={`flex w-full ${isAdminMessage ? 'justify-end' : 'justify-start'}`}>
                                        <div className={`flex items-start gap-3 ${isAdminMessage ? 'flex-row-reverse' : 'flex-row'}`}>
                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold flex-shrink-0 text-sm ${isAdminMessage ? 'bg-primary text-primary-foreground' : 'bg-accent text-accent-foreground'}`}>
                                                {(msg.userName || (isAdminMessage ? 'A' : 'U')).charAt(0).toUpperCase()}
                                            </div>
                                            <div className="flex flex-col max-w-xs sm:max-w-md">
                                                <div className={`flex items-baseline gap-2 ${isAdminMessage ? 'justify-end' : 'justify-start'}`}>
                                                    {isUserMessage && ticket.businessName && <p className="text-xs text-muted-foreground">{ticket.businessName}</p>}
                                                    <p className={`text-xs font-semibold text-foreground`}>
                                                        {isAdminMessage ? "You" : msg.userName}
                                                    </p>
                                                </div>
                                                <div className={`p-3 rounded-lg mt-1 ${isAdminMessage ? 'bg-primary text-primary-foreground' : 'bg-card border border-border'}`}>
                                                    <p className="text-sm whitespace-pre-wrap">{msg.content}</p>
                                                    <p className={`text-xs mt-2 opacity-70 ${isAdminMessage ? 'text-right' : 'text-left'}`}>
                ...
                <CardFooter>
                     {ticket.status === TicketStatus.Closed ? (
                        <p className="text-sm text-muted-foreground w-full text-center">{t('ticketClosedAdmin')}</p>
                    ) : (
