...
const DashboardLayout: React.FC = () => {
    ...
    const handleViewPublicPage = () => {
        if (business?.slug && locations.length > 0) {
            // If there's only one location, go directly to it. Otherwise, go to the location picker.
            const urlSlug = locations.length === 1 ? `${business.slug}/${locations[0].slug}` : business.slug;
            const publicUrl = `/#/b/${urlSlug}`;
            window.open(publicUrl, '_blank');
        } else {
            // Prompt user to set up slug or location if missing
            navigate('/dashboard/settings/details');
        }
    };
    ...
    ].filter(item => item.to !== '/dashboard/templates');
